...
import { limitedConfetti } from '../assets/confettiLimiter.js';

export default function TargetCard({ data, onClick, onLog, onView }) {
  const [flashBtn, setFlashBtn] = useState(null);

  // Utility: small, button‑centred confetti burst (now uses limiter)
  function blow(event) {
    ...
    limitedConfetti({
      ...
      spread: 100,
      scalar: 0.7,
      colors: ['#ff4500', '#ff0000', '#ffd700', '#000000'],
      origin: {
    ...
    <motion.article className="card card--horizontal">
      ...
      <div className="card__content">
        ...
        <div className="card__actions">
          <button
            type="button"
            className={`btn btn--kkill motion-scale ${flashBtn === 'k' ? 'flash' : ''}`}
          ...
          <button
            type="button"
            className={`btn btn--mkill motion-scale ${flashBtn === 'm' ? 'flash' : ''}`}
